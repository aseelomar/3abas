<section class="best-seller section-margin">
    <div class="container-content">
        <div class="section-header">
            <h2>@lang('site.best_seller')
               </h2>
            <a href="{{route('site.product.bestSellerProduct')}}">
        ...
        <div class="boxes-products">
            <div class="large-item">
                <div class="rate-text">
                    <span>{{ round(@$bestSellerProduct[0]->rating, 2) }} ({{ @$bestSellerProduct[0]->total_reviews }})</span>
                ...
                <a target="_self" href="{{route('site.showProduct',@$bestSellerProduct[0]->id)}}">
                <img src="{{ asset('images/products/'.@$bestSellerProduct[0]->id .'/'.@$bestSellerProduct[0]->mainPhoto->image) }}" class="img-product" alt="img"/>
                </a>
                    <div class="description">
                    <a target="_self" href="{{route('site.showProduct',@$bestSellerProduct[0]->id)}}">{{@$bestSellerProduct[0]->name}} </a>
                    <p>{{@$bestSellerProduct[0]->description}} </p>
                </div>
            </div>
            <div class="items">
                @foreach(@$bestSellerProduct->skip(1) as $item)
                <div class="item">
                    <div class="img-box">
                        <a target="_self" href="{{route('site.showProduct',@$item->id)}}">
                        <img src="{{ asset('images/products/'.@$item->id .'/'.@$item->mainPhoto->image) }}" class="img-product" alt="img"/>
                        </a>
                    </div>
                    <div class="description">
                        <a target="_self" href="{{route('site.showProduct',@$item->id)}}">{{@$item->description}}</a>
                        <div class="rate-text">
