<section class="bestseller-this-week section-margin">
    <div class="container-content">
        <div class="section-header">
            <h2> @lang('site.best_seller_this_week')ً</h2>
            <a target="_self" href="{{route('site.product.bestSellerThisWeek')}}">
                <span>الكل</span>
                ...
                <div class="swiper-slide">
                    <div class="large-product-box">
                        <div class="img-box">
                            ...
                            <img src="{{ asset('images/products/'.@$item->product_id .'/'.@$item->product->mainPhoto->image) }}" alt="img"/>
                            </a>
                        </div>
                        <div class="details">
                            <div class="title-like">
                                <a  target="_self" href="{{route('site.showProduct',@$item->product_id)}}">{{@$item->product->name}}</a>
                                <button type="button" class="like-button @if(Auth::user() && isset(Auth::user()->fav_product) && in_array($item->id , Auth::user()->fav_product))
                                    active
                                @endif"productId='{{ $item->product_id }}'>
                            ...
                            <div class="price-rate">
                                <h3>{{@$item->product->product_sale_price}}<span style="font-size: x-small;font-weight: 500;">شيكل</span></h3>
                                <div class="rate-text">
