...
@endsection
@section('title',__('site.order_history'))
@section('subTitle',__('site.order_history'))
@section('sub_Title',__('site.order_history'))
@section('content')
                        ...
                        <div class="order-boxes" >
                            ...
                            <div class="order-box">
                                <div class="header-box row col-12 ">
                                    ...
                                    <span class="order-num">العناصر # {{$loop->iteration}}</span>
                                    <span class="order-num"># {{@$order->number_order}}</span>
                                    @include('site.pages.order.include.status')

                                    <button type="button" class="btn btn-outline-success waves-effect waves-light" data-toggle="modal" data-target="#productView{{ @$order->id }}">
                                        عرض الطلب
                                    </button>
                                    <button type="button" class="btn btn-outline-success waves-effect waves-light" data-toggle="modal" data-target="#orderTracking{{ @$order->id }}">
                                        تتبع الطلب
                                    </button>
{{--                                    <button type="button" class="btn-order" onclick="show_product_modal({{ @$order->id }})">عرض الطلب</button>--}}
{{--                                    <button type="button" class="btn-order remove-order-btn">الغاء الطلب</button>--}}
                                    <button type="button"  onclick="window.location='{{ route('contactUs')}}'"   class="btn-order yellow-color">تقديم شكوى</button>
                                </div>

                            </div>
                                <div class="modal fade" id="productView{{ @$order->id }}" tabindex="-1" aria-hidden="true">
                                            ...
                                            <div id="edit_form_holder">
                                                <div class="order-box" style="border-radius: unset; width:revert; margin: auto" >
                                                    <div class="header-box">
                                                        ...
                                                        <span class="order-num">#{{@$order->number_order}}</span>
                                                        @include('site.pages.order.include.status')
{{--                                                        <div class="status underway">قيد التنفيذ</div>--}}
                                                        <span class="price">{{@$order->total}} <span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                                        @if($order->status_id == \App\Models\Status::STATUS_PENDING)
                                                        <button type="button" data-status="{{$order->status_id}}" data-value="{{@$order->id}}" class="btn-order delete_order">الغاء الطلب</button>
                                                            @endif
                                                    </div>
                                                    <div class="order-body">
                                                        <div class="flex-items">
                                                            <div class="info">
                                                                <h3 class="date"> {{\Carbon\Carbon::parse(@$order->created_at)->toFormattedDateString()}}</h3>
                                                                <p>
                                                                ...
                                                                <div class="items">
                                                                    ...
                                                                    <div class="item">
                                                                        <div class="head">
                                                                            <a href="{{route('site.showProduct',@$item->cart->product->id ?? 0)}}">
                                                                            <h4>{{@$item->cart->product->product_name_ar}} </h4>
                                                                            <span style="font-weight: bold;     text-align: center;"> السعر النهائي{{@$item->cart->total_price}}<span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                                                            </a>
                                                                        </div>
                                                                        <p>{{@$item->cart->product_sale_price}}*{{@$item->cart->count}}= {{@$item->cart->price_before_discount}} <span style="font-size: x-small;font-weight: 500;">شيكل</span></p>
                                                                    </div>
                                                            ...
                                                            <div class="calculator">
                                                                <div class="items">
                                                                    <div class="item">
                                                                        <span>سعر الطلب</span>
                                                                        <span>{{@$order->price_before_discount}}<span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                                                    </div>
                                                                    <div class="item">
                                                                    ...
                                                                    <div class="item">
                                                                        ...
                                                                        <span>{{@$order->discount}}<span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                                                    </div>
                                                                    <div class="item">
                                                                        <span class="bold-text">المجموع الكلي</span>
                                                                        <span>{{@$order->total}}<span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                ...
                                <div class="modal fade" id="orderTracking{{ @$order->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered mw-650px">
                                                ...
                                                <div class="order-box" style="border-radius: unset; width: max-content; margin: auto" >
                                                    <div class="table-responsive">
                                                        <table class="table table-striped mb-0">
                                                            ...
                                                            <tbody>
                                                            @foreach($order->orderTracking  as $item)
                                                                <tr>
                        ...
                        <a id="redirict" href="{{route('site.order.orderHistory')}}" hidden></a>
                    </div>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.delete_order', function(){
            ...
            swal.fire({
                ...
                if (result.value){
                    $.ajax({
                               url: '{{route('site.order.deleteOrder')}}',
                               type: 'POST',
                               data: {
                                   _token: '{{csrf_token()}}',
                                   id:id,
