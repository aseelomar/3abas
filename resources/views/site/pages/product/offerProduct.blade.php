...
@section('subTitle',__('site.offers'))
@section('sub_Title',__('site.offers'))
@section('content')
                    ...
                    @foreach(@$featuredProduct as $item)
                        <div class="category-box">
                            <div class="img-box">
                                ...
                                <img src="{{ asset('images/products/'.@$item->id .'/'.@$item->mainPhoto->image) }}" alt="category"/>
                                </a>
                            </div>
                            <div class="details">
                                <div class="content">
                                    <a  target="_self"href="{{route('site.showProduct',@$item->id)}}" class="title">
                                        <h3>{{@$item->name}}</h3>
                                        <p>( <span>Rate</span> )</p>
                                    </a>
                                    <p class="description">{{@$item->description}}</p>
                                    <div class="price-and-add-to-cart">
                                        <span class="price">{{@$item->product_sale_price}} <span style="font-size: x-small;font-weight: 500;">شيكل</span></span>
                                        @if($item->details->isEmpty())
                                            <form  class="form-add-car" data-value="{{@$item->id}}" method="POST" style="DISPLAY: CONTENTS;">
                                                @csrf
                                                <input type="hidden" value="{{@$item->id}}" name="product_id" class="product_add_cart_id">
